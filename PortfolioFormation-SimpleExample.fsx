...
let signals =
    ...
    |> Map
signals[Ticker "AAPL"]
Map.find (Ticker "AAPL") signals(* output: 
2.0*)
Map.tryFind (Ticker "AAPL") signals(* output: 
Some 2.0*)
Map.tryFind (Ticker "GME") signals(* output: 
Real: 00:00:00.038, CPU: 00:00:00.031, GC gen0: 1, gen1: 1, gen2: 0
...
*)
getSecuritySignal (Ticker "GME")(* output: 
Real: 00:00:00.088, CPU: 00:00:00.078, GC gen0: 0, gen1: 0, gen2: 0
val it: SecuritySignal option = None*)
getSecuritySignal (Ticker "GOOG")(* output: 
...
getPortfolioIdString portfolioId1(* output: 
"Market"*)
getPortfolioIdString portfolioId3(* output: 
"Size: 2"*)
(**
...
*)
giveEqualWeights weightTestPort(* output: 
input.fsx (1,1)-(1,17) typecheck error The value or constructor 'giveEqualWeights' is not defined. Maybe you want one of the following:
   giveValueWeights*)
(**
For all portfolios:

*)
[ for portfolio in assigned do giveEqualWeights portfolio ](* output: 
input.fsx (1,20)-(1,28) typecheck error The value or constructor 'assigned' is not defined. Maybe you want one of the following:
   AssignedPortfolio
   ascending
   assignSignalSort
   Assert
input.fsx (1,32)-(1,48) typecheck error The value or constructor 'giveEqualWeights' is not defined. Maybe you want one of the following:
   giveValueWeights*)
(**
or equivalently:

*)
assigned |> List.map giveEqualWeights(* output: 
input.fsx (1,1)-(1,9) typecheck error The value or constructor 'assigned' is not defined. Maybe you want one of the following:
   AssignedPortfolio
   ascending
   assignSignalSort
   Assert
input.fsx (1,22)-(1,38) typecheck error The value or constructor 'giveEqualWeights' is not defined. Maybe you want one of the following:
   giveValueWeights*)
(**
...
let mktCaps =
    [ for signal in weightTestPort.Signals do 
        ...
        signal.SecurityId, mktcap ](* output: 
input.fsx (9,21)-(9,35) typecheck error The value, namespace, type or module 'weightTestPort' is not defined. Maybe you want one of the following:
   weightsXreturn*)
let vwTestWeights =
    let totMktCap = mktCaps |> List.sumBy snd
    [ for (id, mktCap) in mktCaps do 
        { SecurityId = id 
          Weight = mktCap / totMktCap } ](* output: 
input.fsx (2,21)-(2,28) typecheck error The value or constructor 'mktCaps' is not defined.
input.fsx (3,27)-(3,34) typecheck error The value or constructor 'mktCaps' is not defined.*)
(**
...
*)
[ for x in assigned do giveValueWeights x ](* output: 
No value returned by any evaluator*)
(**
or equivalently

*)
assigned |> List.map giveValueWeights  (* output: 
No value returned by any evaluator*)
(**
...
let strategyWeights =
    ...
    |> List.map giveValueWeights(* output: 
input.fsx (4,8)-(4,30) typecheck error The value or constructor 'assignAboveBelowMedian' is not defined.*)
(**
