...
|> List.take 3(* output: 
input.fsx (1,1)-(1,26) typecheck error The value or constructor 'standardInvestmentsExcess' is not defined.*)
ff3 
|> List.filter(fun x -> x.Date.Year = 2021)
|> List.map(fun x -> x.Date.Month, round 4 x.MktRf)
|> List.take 3(* output: 
input.fsx (1,1)-(4,15) interactive warning Accessing the internal type, method or field 'Date@' from a previous evaluation in F# Interactive is deprecated and may cause subsequent access errors. To enable the legacy generation of a single dynamic assembly that can access internals, use the '--multiemit-' option.
input.fsx (1,1)-(4,15) interactive warning Accessing the internal type, method or field 'MktRf@' from a previous evaluation in F# Interactive is deprecated and may cause subsequent access errors. To enable the legacy generation of a single dynamic assembly that can access internals, use the '--multiemit-' option.*)
(**
